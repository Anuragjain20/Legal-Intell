...
**The live `/query` path runs `HybridQueryRetriever` (dense + BM25, RRF-fused), not the plain dense `Retriever` described below** — hybrid measurably outperforms dense-only (Recall@5 0.7838 vs 0.6486, see [06-evaluation.md](06-evaluation.md)). This file still documents `Retriever.retrieve()` in detail because it's the simpler mental model to build up from, it's still directly used by `scripts/run_evaluation.py --method dense` for comparison, and `HybridRetriever` calls it internally as its dense half. The differences that matter for the live path: `HybridQueryRetriever` fuses dense and BM25 rankings via Reciprocal Rank Fusion instead of using dense similarity alone, and gates confidence on the fused result's `dense_score` field (not the RRF score, which isn't a cosine similarity) — see [01-architecture.md](01-architecture.md) and `src/retrieval/hybrid_query_retriever.py`.
...
**Why not a cross-encoder reranker (e.g. a `bge-reranker` or Cohere rerank call)?** Two honest reasons to give: (1) scope — this is an MVP wired for a fixed, small, known corpus, and a hand-rolled heuristic that encodes one specific domain insight (purpose questions → look at frontmatter) was cheaper to build and easier to unit test than adding another model dependency; (2) it works entirely offline/deterministically, no extra latency or API cost per query. The honest trade-off to name back: this heuristic doesn't generalize past the patterns it was written for.

**A different, measurable reranker was actually tried and rejected.** `src/retrieval/reranking.py`'s `QueryTermOverlapReranker`, applied on top of hybrid retrieval, was evaluated against the same 42-question dataset as every other method (`python scripts/run_evaluation.py --method hybrid_rerank`) — and it made results *worse*: Recall@5 dropped from hybrid's 0.7838 to 0.6216, worse than dense alone. It is not wired into `/query`. This is the strongest answer to "what would you add first" — not "a cross-encoder," but "I tried a reranker, measured it, and it made things worse, so the honest next step is understanding *why* before reaching for a bigger model." See [06-evaluation.md](06-evaluation.md) for the full table and [07](07-interview-narrative.md)/[08](08-question-bank.md) for how to talk about it.
...
Metadata filters (category, document, date range) are a `NotImplementedError` stub on both `Retriever.retrieve(..., filters=None)` ([retriever.py:24-25](../src/retrieval/retriever.py#L24-L25)) and `HybridQueryRetriever.retrieve(..., filters=None)`, despite `Chunk`/`VectorRecord` already carrying `category`, `document_id`, `section` fields that a real filter implementation could use directly against Chroma's `where=` clause (already used internally for `delete_document`, [chroma_store.py:48](../src/vectorstore/chroma_store.py#L48)). This is a "next thing to build," not a "thing that doesn't exist as a concept" — the metadata is already there.
