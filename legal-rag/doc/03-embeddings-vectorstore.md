...
- Runs **locally, in-process** — no external embedding API call, no per-call cost or latency to a remote service. This is a deliberate MVP trade-off: free and simple, but the model lives in the FastAPI backend's memory, loaded once at startup by `build_services()` and held for the process's lifetime (see [01-architecture.md](01-architecture.md) on scaling).
