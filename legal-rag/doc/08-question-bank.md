...
Recall@1/3/5 and MRR, computed by `RetrievalEvaluator` against a 22-question dataset (17 in-corpus, 5 out-of-corpus). Real last-run numbers: Recall@1 0.588, Recall@3 0.765, Recall@5 0.824, MRR 0.669 — pulled directly from `data/evaluation_results.json`, reproducible via `python run_evaluation_real.py`.

**Q: ⚠️ Is "Recall@K" the right name for what you're measuring?**
Not strictly — every question in my dataset has exactly one relevant chunk, so what I'm computing is Hit-Rate@K (did the one relevant item appear in the top K), which only coincides with textbook Recall@K (fraction of *all* relevant items retrieved) in the single-relevant-item case. I'd rename the metric if I were writing this up formally. Good catch if you're testing whether I understand the metric rather than just reporting it.

**Q: ⚠️ How were your evaluation questions generated — are they realistic?**
No, and I'd rather say that plainly than get caught overclaiming. `build_valid_evaluation_dataset.py` templates each in-corpus question directly from the chunk it's testing — e.g. `"What does the {section} section state about {heading}?"`, using that exact chunk's own metadata. It's grounded (every question maps to a real indexed chunk, not an invented one) but it's not independent of the system's own indexing — it's closer to "is this chunk retrievable using its own metadata as a query" than "how does this handle a real user's phrasing." A 0.59 Recall@1 on queries this favorable suggests real headroom in retrieval or chunking, since this isn't testing phrasing robustness at all.

**Q: ⚠️ What does `no_answer_detection_rate` measure?**
I'll be direct: as currently implemented, it's a bug. It computes `out_of_corpus_count / total_count` — a fact about how many out-of-corpus questions I put in the dataset, fixed before any retrieval ever runs. Worse, `_evaluate_single()` returns early for out-of-corpus cases without ever calling `retriever.retrieve()` on them ([evaluator.py:92-109](../src/evaluation/evaluator.py#L92-L109)) — so a metric whose name promises "did the system detect it had no answer" never actually checks the system's behavior at all. And I didn't stop at diagnosing the bug — the raw data to compute the real metric already exists (`build_valid_evaluation_dataset.py` runs those 5 questions through the live retriever at dataset-build time and logs the results, `evaluator.py` just never reads that field), so I read it myself: all 5 out-of-corpus questions scored 0.62–0.66, above my 0.35 threshold. **The real detection rate is 0/5, not the 0.227 the current metric implies.** My system currently never abstains on an out-of-corpus question — it always finds *something* above threshold and hands it to the LLM as context. The fix for the metric itself is straightforward (call the retriever inside a try/except, count `NoRelevantResultsError` as a true negative), but the more important finding is the one that fix would surface: the similarity threshold is too low to actually gate irrelevant queries on this corpus.

**Q: ⚠️ I noticed your evaluation script prints "Correctly identified as out-of-corpus" — is that accurate?**
No, and I'd rather flag it than have it discovered. `run_evaluation_real.py`'s failure-analysis section prints that exact line for every out-of-corpus test case unconditionally ([run_evaluation_real.py:104-108](../run_evaluation_real.py#L104-L108)) — it doesn't check anything, it labels every out-of-corpus case a success by construction, same underlying bug as `no_answer_detection_rate`. Per the data above, the accurate statement is the opposite: none of the five were correctly identified as out-of-corpus, they all scored above threshold and would have been treated as relevant.
...
111 tests, 107 passing, 4 failing. One failure is a stale string assertion in a generation test that wasn't updated after the prompt wording changed. Three are structure-detector/chunker edge cases in the regex cascade — I can describe the exact input that trips each one: `"CHAPTER I\n10. Short title"` (the chapter heading incorrectly absorbs the next line instead of yielding a clean "Short title" heading); a bare `"Section 1"` line that isn't matched by any pattern at all, so it's never detected as a heading; and a lettered-subsection case where the section's stored body text ends up one paragraph off from what's expected — the title line gets absorbed into the heading and the wrong paragraph becomes the body.
